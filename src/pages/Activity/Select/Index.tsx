...
const Select = () => {
  ...
  const [activitys, setActivitys] = useState<ActivityProps[] | null>(null)
  ...
  useEffect(() => {
    const getActivitys = async () => {
      try {
        const { data } = await axios.get(`activity/select/all${location.search}`, {
  ...
  return (
    <>
      <Breadcrumb pageName="Listar Atividades" />
